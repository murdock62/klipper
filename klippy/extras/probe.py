...
class PrinterProbe:
    ...
    def _probe(self, speed):
        ...
        # if x_twist_compensation is active
        # get the z compensation
        x_twist_compensation = self.printer.lookup_object(
            'x_twist_compensation', None)
        z_compensation_enabled = \
            x_twist_compensation is not None \
            and x_twist_compensation.pmgr.get_is_enabled()
        z_compensation = 0 if not z_compensation_enabled else x_twist_compensation.get_z_compensation_value(
            pos[0])
        logging.info("probe z_twist_compensation enabled: %s, z_compensation: %s, passed x_coord: %s",
                     z_compensation_enabled, z_compensation, pos[0])
